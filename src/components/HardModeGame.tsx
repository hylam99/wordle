...
import { HardModeClientService } from "@/services/hardModeClientService";
import { ConfigManager } from "@/services/configManager";
import { defaultConfig, GameConfig } from "@/config/gameConfig";
import { GuessResult } from "@/types/game";
import { HardModeClientState } from "@/services/hardModeGameService";
...
export default function HardModeGame() {
  ...
  const [clientGameService] = useState(() => new HardModeClientService());
  const [guess, setGuess] = useState("");
  const [results, setResults] = useState<GuessResult[][]>([]);
  const [gameState, setGameState] = useState<HardModeClientState | null>(null);
  ...
  useEffect(() => {
    const initializeGame = async () => {
      setLoading(true);
      try {
        const initialGameState = await clientGameService.createGame(
  ...
  const handleKeyPress = useCallback(
    (key: string) => {
      if (!gameState || gameState.gameOver || guess.length >= 5 || loading)
        return;
  ...
  const handleReset = useCallback(async () => {
    ...
    try {
      const newGameState = await clientGameService.resetGame(
        configManager.getConfig()
      );
      setGameState(newGameState);
      setResults([]);
            ...
            <div key={roundIndex} className="flex gap-1 justify-center">
              {Array.from({ length: 5 }, (_, letterIndex) => {
                ...
                const isCurrentRound =
                  roundIndex === gameState.currentRound && !gameState.gameOver;
                const currentGuessLetter = isCurrentRound
                  ? guess[letterIndex]
                  : null;
                ...
                return (
                  ...
                  >
                    {letter
                      ...
                      : currentGuessLetter?.toUpperCase() || ""}
                  </div>
                );
              })}
            </div>
        ...
        {gameState.gameOver && (
          <div className="text-center space-y-4 mb-6">
            ...
            <div className="text-lg">
              {gameState.won
                ? `You won in ${gameState.currentRound} ${
                    gameState.currentRound === 1 ? "guess" : "guesses"
                  }!`
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-green-500 text-white text-xs font-bold flex items-center justify-center">
              G
            </div>
            <span>
              <strong>Hit:</strong> Letter is in the correct spot
            </span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-yellow-500 text-white text-xs font-bold flex items-center justify-center">
              Y
            </div>
            <span>
              <strong>Present:</strong> Letter is in the word but wrong spot
            </span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-gray-400 text-white text-xs font-bold flex items-center justify-center">
              G
            </div>
            <span>
              <strong>Miss:</strong> Letter is not in the word
            </span>
          </div>
        </div>
      </div>
