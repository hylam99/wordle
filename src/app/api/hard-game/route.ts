...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error getting hard mode game state:', error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const gameState = HardModeGameService.resetGame(gameId, config);
    
    return NextResponse.json({ gameState });
    
  } catch (error) {
    console.error('Error resetting hard mode game:', error);
